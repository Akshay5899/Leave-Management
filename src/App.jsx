...
function App() {
  ...
  useEffect(() => { if (session) { loadRequests(); loadEmployees(); } }, [session]);
  ...
  const logout = () => { localStorage.removeItem('leaveflow_session'); setSession(null); setRequests([]); setEmployees([]); };

  if (!session) return <Login onLogin={login} />;

  return <main className="app-shell">
    <aside className="sidebar"><div className="side-logo">LF</div><nav><button className="side-link active" aria-label="Dashboard">⌂</button><button className="side-link" aria-label="Leave requests">▦</button><button className="side-link" aria-label="Employees">♙</button><button className="side-link" aria-label="Messages">•••</button></nav><button className="side-link logout" aria-label="Log out">↪</button></aside>
...
function Login({ onLogin }) {
  ...
  const submit = async event => { event.preventDefault(); setLoading(true); setError(''); try { await onLogin(form); } catch (loginError) { setError(loginError.message); setLoading(false); } };
  return <main className="login-page"><div className="login-art"><div className="login-logo">LF</div><p className="eyebrow">Employee leave management</p><h1>Time off,<br /><em>made clear.</em></h1><p>One calm place for teams to request, review, and track leave.</p></div><form className="login-card" onSubmit={submit}><div className="login-card-head"><p className="eyebrow">Welcome back</p><h2>Sign in to Leaveflow</h2><p>Use your work account to continue.</p></div><label>Email address<input required type="email" name="email" value={form.email} onChange={update} placeholder="you@company.com" autoComplete="email" /></label><label>Password<input required type="password" name="password" value={form.password} onChange={update} placeholder="Enter your password" autoComplete="current-password" /></label>{error && <p className="form-error">{error}</p>}<button className="primary-button login-submit" disabled={loading}>{loading ? 'Signing in...' : 'Sign in'}</button><p className="login-hint">Manager and employee access use separate permissions.</p></form></main>;
}
